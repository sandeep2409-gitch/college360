...
export default function TimeTablePage() {
  ...
  const fetchTimetable = async (className) => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(`/api/timetable?className=${className}`);
      if (response.data) {
  ...
  const saveTimetable = async () => {
    try {
      setLoading(true);
      await axios.post('/api/timetable', { className: selectedClass, data: schedule }, { headers: { Authorization: `Bearer ${token}` } });
      setIsEditing(false);
      fetchClasses();
  ...
  const handleAddClass = () => {
    ...
    if (classes.includes(newClassName)) { alert("Class already exists"); return; }
    ...
    <div className="timetable-page animate-slide-up" style={{ padding: '20px' }}>
      <header style={{ marginBottom: '30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <h1 style={{ fontSize: '2rem' }}>Class Time Table</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginTop: '10px' }}>
            <span style={{ fontSize: '0.9rem', color: 'var(--text-dim)', fontWeight: '600' }}>Selected Class:</span>
            <select value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-main)', padding: '6px 12px', borderRadius: '8px', fontSize: '0.9rem', minWidth: '200px' }}>
            ...
            {isAdmin && (
              <button onClick={() => setShowAddClassModal(true)} className="btn-primary" style={{ padding: '6px 12px', fontSize: '0.8rem', background: '#334155' }}>
                + New Class
              </button>
            )}
          </div>
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          {isAdmin && selectedClass && (
            <button onClick={() => isEditing ? saveTimetable() : setIsEditing(true)} disabled={loading} className="btn-primary" style={{ background: isEditing ? '#22c55e' : 'var(--primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              {loading ? <Loader2 size={18} className="animate-spin" /> : (isEditing ? <Check size={18} /> : <Edit2 size={18} />)}
              {isEditing ? 'Save Changes' : 'Edit Schedule'}
            </button>
          )}
          <button className="btn-primary" onClick={() => window.print()} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#334155' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0, 0, 0, 0.45)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', zIndex: 2000, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div className="glass-card animate-fade-in" style={{ width: '90%', maxWidth: '400px', padding: '30px', border: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <input type="text" placeholder="e.g. IV-B.Tech-CSE-A" className="input-field" value={newClassName} onChange={(e) => setNewClassName(e.target.value)} />
