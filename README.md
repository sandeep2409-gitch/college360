...
"Our project utilizes a **PERN-inspired stack** (Postgres replaced by SQLite for local portability). The frontend is a **Single Page Application (SPA)** that communicates with a **RESTful API** on the backend. We've integrated **Generative AI** using the Google Gemini model to provide students with a context-aware campus assistant, bridging the gap between static data and interactive AI."
