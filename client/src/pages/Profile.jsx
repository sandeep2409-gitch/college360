...
import { User, Mail, Shield, Award, Calendar, Clock, BookOpen, Camera, Check, Edit2, LogOut, ChevronRight, Settings, Loader2 } from 'lucide-react';
...
const Profile = () => {
    ...
    const fetchProfileStats = async () => {
        try {
            ...
            // Fetch attendance
            const attRes = await axios.get(`${import.meta.env.VITE_API_URL}/api/attendance/${user.id}`, config);
            const attendanceCount = Array.isArray(attRes.data) ? attRes.data.length : 0;

            // In a real app we'd fetch more detailed stats, but let's derive from available data
            setStats({
                attendance: attendanceCount,
                resources: user.role === 'admin' ? 'All' : '5+', // Mock derived
                complaints: 0
            });
        } catch (error) {
    ...
    return (
        <div className="profile-page animate-slide-up" style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '32px' }}>
                ...
                <div className="glass-card" style={{ padding: '32px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                        ...
                        <button onClick={logout} style={{ marginTop: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', width: '100%', padding: '16px', border: '1px solid var(--danger)', color: 'var(--danger)', background: 'transparent', borderRadius: '16px', fontWeight: '800', cursor: 'pointer', transition: '0.3s' }}>
